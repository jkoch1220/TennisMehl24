...
const ZahlungsSchnelleingabe = ({ rechnung, onUpdate }: ZahlungsSchnelleingabeProps) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-lg shadow-lg dark:shadow-slate-900/50 p-4 border-2 border-gray-200 dark:border-slate-700">
      {/* Kopf mit Stand heute */}
      <div className="mb-4 pb-4 border-b border-gray-200 dark:border-slate-700">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <Euro className="w-5 h-5 text-green-600" />
            <h4 className="font-semibold text-gray-900 dark:text-slate-100">Zahlungsstand</h4>
          </div>
          {heuteBezahlt > 0 && (
        ...
        <div className="grid grid-cols-3 gap-4 mt-3">
          <div>
            <div className="text-xs text-gray-500 dark:text-slate-400 mb-1">Rechnungssumme</div>
            <div className="text-lg font-bold text-gray-900 dark:text-slate-100">{formatCurrency(rechnung.summe)}</div>
          </div>
          <div>
            <div className="text-xs text-gray-500 dark:text-slate-400 mb-1">Bereits bezahlt</div>
            <div className="text-lg font-bold text-green-600">{formatCurrency(gesamtBezahlt)}</div>
          </div>
          <div>
            <div className="text-xs text-gray-500 dark:text-slate-400 mb-1">Noch offen</div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-4 py-2 border border-gray-300 dark:border-slate-700 text-gray-700 dark:text-slate-400 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-800 transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
      ...
      {rechnung.zahlungen && rechnung.zahlungen.length > 0 && (
        <div className="mt-4 pt-4 border-t border-gray-200 dark:border-slate-700">
          <div className="text-xs font-medium text-gray-700 dark:text-slate-400 mb-2">Zahlungshistorie</div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      {zahlung.notiz && (
                        <span className="text-xs text-gray-500 dark:text-slate-400">- {zahlung.notiz}</span>
                      )}
                    </div>
