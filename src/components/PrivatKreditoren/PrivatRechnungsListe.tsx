...
import { OffeneRechnung, RechnungsFilter, SortierFeld, SortierRichtung, RechnungsStatus, Rechnungskategorie, Prioritaet } from '../../types/kreditor';
import { getRelevanteFaelligkeit, istRechnungHeuteFaellig } from '../../utils/ratenzahlungCalculations';
import PrivatZahlungsSchnelleingabe from './PrivatZahlungsSchnelleingabe';

interface PrivatRechnungsListeProps {
...
const PrivatRechnungsListe = ({ rechnungen, onEdit, onDelete, onRefresh, onOpenDetail }: PrivatRechnungsListeProps) => {
  ...
  const getStatusBadge = (status: RechnungsStatus) => {
    const styles: Record<RechnungsStatus, string> = {
      offen: 'bg-purple-100 text-purple-800',
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
            />
          </div>
                ...
                displayedRechnungen.map((rechnung) => {
                  ...
                  return (
                    <>
                      <tr
                        ...
                        className={`cursor-pointer transition-colors ${
                          istUeberfaellig && rechnung.status !== 'bezahlt' && rechnung.status !== 'storniert'
                            ...
                            : 'bg-white dark:bg-slate-800 hover:bg-purple-50 dark:hover:bg-purple-900/20'
                        }`}
                      >
                      ...
                      {expandedRows.has(rechnung.id) && (
                        <tr key={`${rechnung.id}-expanded`} className="bg-purple-50 dark:bg-purple-900/20">
                          <td colSpan={8} className="px-6 py-4">
                            <PrivatZahlungsSchnelleingabe rechnung={rechnung} onUpdate={onRefresh} />
                          </td>
                        </tr>
                      )}
                    </>
...
export default PrivatRechnungsListe;
